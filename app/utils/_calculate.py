# -*- coding: utf-8 -*-
"""
Created on Wed Jan 25 15:09:23 2023
...
def auc_estimation(y, baseline, peaks):
    auc_all = []
    #y=trace_win
    #baseline=base_line
    #peaks=peaks
    for i in range(1,len(peaks)):        
        try:
            sig_link = y[1:peaks[i]]
            baselin_link = baseline[1:peaks[i]]
            link_ind = np.where(baselin_link >= sig_link)[0][-1]
        except IndexError:
            auc_peak = 0
            auc_all.append(auc_peak)
            continue
                
        try:
            sig_recht = y[peaks[i]:len(y)]
            baselin_recht = baseline[peaks[i]:len(y)]
            recht_ind = np.where(baselin_recht >= sig_recht)[0][0]
        except IndexError:
            auc_peak = 0
            auc_all.append(auc_peak)
            continue
            
        baseline_peak = baseline[link_ind:(recht_ind+peaks[i])]
        signal_peak = y[link_ind:(recht_ind+peaks[i])]
        x=range(link_ind,(recht_ind+peaks[i]))
    
        auc_base = metrics.auc(x, baseline_peak)
        auc_signal = metrics.auc(x, signal_peak)
        auc_peak = auc_signal-auc_base
        auc_all.append(auc_peak)
...
with open('C:/Users/JuanPrada/Downloads/data+(1).pkl', 'rb') as f:
    data = pickle.load(f)
        
rowDim = np.shape(list(data.values())[0])[0] 
colDim = np.shape(list(data.values())[0])[1] 
winSize = 12

data_convolved = []
#ind_datacon = 1
for items_data in data.values():
    #data_convolved[ind_datacon] = np.mean(items_data)
    #data_convolved.append(np.mean(items_data))
    #print(items_data.shape)
    img_temp = scipy.ndimage.uniform_filter(items_data,size=12)
    #data_convolved.append(zoom(img_temp,1/winSize))
    #data_convolved.append(cv2.resize(img_temp, dsize=(np.ceil(rowDim/winSize), 
#                        np.ceil(colDim/winSize)), interpolation=cv2.INTER_CUBIC))
    data_convolved.append(resize(img_temp, (np.ceil(rowDim/winSize), 
                        np.ceil(colDim/winSize))))


traces=[]
results=[]
#num_elem_array=data_convolved[0].shape[0]*data_convolved[0].shape[1]
widths_cwt = np.linspace(1,len(data)/10,num=1000)
max_distances = widths_cwt/10
gap_thresh = np.ceil(widths_cwt[0])
min_snr = 1
min_length = len(widths_cwt)*0.125
noise_perc = 5
        
for ind_col in range(0,data_convolved[0].shape[1]):
    for ind_row in range(0,data_convolved[0].shape[0]):    
#for ind_col in range(6,8):
#    for ind_row in range(7,9):    
        trace_win = [0]*len(data)
        ind_win = ind_col*data_convolved[0].shape[0] + ind_row
        for ind_frame in range(0,len(data)):     
            trace_win[ind_frame] = data_convolved[ind_frame][ind_row,ind_col,0]
        cwtmatr = cwt(trace_win, ricker, widths_cwt)
        ridge_lines = _identify_ridge_lines(cwtmatr, max_distances, gap_thresh)
        filtered = _filter_ridge_lines(cwtmatr, ridge_lines, min_length=min_length,
                                    min_snr=min_snr, noise_perc=noise_perc)
        filtered_list = list(filtered)
        peaks = find_peaks_cwt(trace_win, widths=widths_cwt,max_distances=max_distances,gap_thresh=gap_thresh,
                     min_length=min_length,min_snr=min_snr,noise_perc=noise_perc)
        traces.append(trace_win)
        base_line = baseline_als(trace_win,lam=10000000,p=0.05)
        auc_peaks = auc_estimation(trace_win,base_line,peaks)
        res_list = [trace_win,base_line,peaks,auc_peaks]
